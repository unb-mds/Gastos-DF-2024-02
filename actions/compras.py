    ...
    for gazette in dados["gazettes"]:
        ...
        for excerto in gazette["excerpts"]:
            ...
            empresa_match = re.search(
                r"empresa\s+([\w\s\-ÇçÉéÁáÍíÓóÚúÃãÕõâêîôûÂÊÎÔÛäëïöüÄËÏÖÜ]+)\s*-\s*CNPJ:\s*(\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2})",
                excerto,
                re.IGNORECASE,
            )
            objeto_match = re.search(
                r"cujo\s+objeto\s+é\s+a\s+aquisição\s+do\s+item\s+identificado\s+pelo\s+Código\s+SES\s+\d+\s*-\s*([^\n,]+)",
                excerto,
                re.IGNORECASE,
            )
            valor_match = re.search(
                r"valor\s+global\s+de\s+R\$\s*([\d.,]+)",
                excerto,
                re.IGNORECASE,
            )
            if empresa_match and objeto_match and valor_match:
                ...
                objeto = re.sub(
                    r",\s*para\s+atender\s+as\s+necessidades.*",
                    "",
                    objeto_match.group(1),
                    flags=re.IGNORECASE,
                )
