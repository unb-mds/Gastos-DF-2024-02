...
class ProcessadorDados:
    ...
    def processar_dados_compras(self) -> Dict[str, List]:
        ...
        for data, itens in dados_compras.items():
            ...
            total_pago = sum(
                converter_valor_monetario(item["Valor"]) 
                for item in itens
            )
        ...
        meses_ordenados = ordenar_meses_cronologicamente(list(gastos_mensais.keys()))
    ...
    def processar_dados_despesas(self) -> Dict[str, Dict[str, List]]:
        ...
        for arquivo in arquivos:
            ...
            # Calcular médias
            empenhado_media = sum(converter_valor_monetario(item["empenhado"]) 
                                   for item in dados) / len(anos)
            liquidado_media = sum(converter_valor_monetario(item["liquidado"]) 
                                   for item in dados) / len(anos)
            pago_media = sum(converter_valor_monetario(item["pago"]) 
                              for item in dados) / len(anos)
            ...
            for ano in anos:
                ...
                empenhado_val = sum(converter_valor_monetario(item["empenhado"]) 
                                     for item in dados if item["ano"] == ano)
                liquidado_val = sum(converter_valor_monetario(item["liquidado"]) 
                                     for item in dados if item["ano"] == ano)
                pago_val = sum(converter_valor_monetario(item["pago"]) 
                                for item in dados if item["ano"] == ano)
